...
# RNN_folder = '0.01811_ColorDiscrimination;relu;N=99;lmbdo=0.3;lmbds=0.1;lmbdr=0.5;lr=0.005;maxiter=3000'
RNN_folder = '0.017999_ColorDiscrimination;relu;N=98;lmbdo=0.3;lmbds=0.1;lmbdr=0.5;lr=0.005;maxiter=3000'
...
for trial in range(20):
    ...
    penalty_type = LCI_config_file["recurrent_layer_penalty"]
    ...
    task = TaskColorDiscrimination(n_steps=n_steps, task_params=task_data)
    ...
    match actvation_name_LC:
        case "relu": activation_fun_circuit = lambda x: np.maximum(x, 0)
        case "tanh": activation_fun_circuit = lambda x: np.tanh(x)
        case "sigmoid": activation_fun_circuit = lambda x: 1/(1 + np.exp(-x))
        case "softplus": activation_fun_circuit = lambda x: np.log(1 + np.exp(5 * x))
    ...
    if r2_tot > 0.82:
        ...
        w_inp = net_params["W_inp"]
        fig_w_inp = analyzer.plot_input_matrix()
        datasaver.save_figure(fig_w_inp, f"{r2_tot}_{r2_proj}_LC_winp.png")
        if disp: plt.show()
