...
from rnn_coach.src.Tasks.TaskDMTS import *
...
RNN_folder = '0.0003952_DMTS;relu;N=98;lmbdo=0.3;orth_inp_only=True;lmbdr=0.4;lr=0.0005;maxiter=5000'
tag = 'DMTS'
...
# task_data["n_steps"] = 100
# task_data["stim_on_sample"] = 0
# task_data["stim_off_match"] = 10
# task_data["stim_on_match"] = 20
# task_data["stim_off_match"] = 30
# task_data["dec_on"] = 40
# task_data["dec_off"] = 100
# mask = np.concatenate([np.arange(task_data["dec_on"]), task_data["dec_on"] + 20 +  np.arange(40)])
...
for trial in range(15):
    ...
    w_rec = np.array(LCI_config_file["W_rec"])
    dale_mask = np.array(LCI_config_file["dale_mask"])
    ...
    task = TaskDMTS(n_steps=n_steps, task_params=task_data, n_inputs=3, n_outputs=2)

    lc = LatentCircuit(N=N_LC,
                       W_inp=torch.Tensor(w_inp).to(device),
                       W_out=torch.Tensor(w_out).to(device),
                       W_rec=torch.Tensor(w_rec).to(device),
    ...
    circuit.run(input_batch_valid)
    output_valid = circuit.get_output()
    #MSE mse_score_RNN
    score_function = lambda x, y: np.mean((x - y) ** 2)
    mse_score = analyzer.get_validation_score(scoring_function=mse_scoring,
                                              input_batch=input_batch_valid,
                                              target_batch=output_valid,#target_batch_valid,
                                              mask=mask,
                                              sigma_rec=sigma_rec,
                                              sigma_inp=sigma_inp)
    ...
    # if r2_tot > 0.82:
    data_save_folder = os.path.join(data_save_path, RNN_folder, f"{N_LC}_{mse_score}_{r2_tot}_{r2_proj}_LC_{tag}")
    datasaver = DataSaver(data_save_folder)
    datasaver.save_data(scores, f"{mse_score}_{r2_tot}_{r2_proj}_LC_scores.json")
    datasaver.save_data(jsonify(LCI_config_file), f"{mse_score}_{r2_tot}_{r2_proj}_LC_config.json")
    datasaver.save_data(jsonify(net_params), f"{mse_score}_{r2_tot}_{r2_proj}_LC_params.json")
    ...
    datasaver.save_figure(fig_w_rec, f"{mse_score}_{r2_tot}_{r2_proj}_LC_wrec.png")
    if disp: plt.show()
    ...
    datasaver.save_figure(fig_circuit, f"{mse_score}_{r2_tot}_{r2_proj}_LC_circuit.png")
    if disp: plt.show()

    w_out = net_params["W_out"]
    fig_w_out = analyzer.plot_output_matrix()
    datasaver.save_figure(fig_w_out, f"{mse_score}_{r2_tot}_{r2_proj}_LC_wout.png")
    if disp: plt.show()

    fig_w_rec_comparison = analyzer.plot_recurrent_matrix_comparison(w_rec_bar=w_rec_bar)
    datasaver.save_figure(fig_w_rec_comparison, f"{mse_score}_{r2_tot}_{r2_proj}_LC_wrec_comparison.png")
    if disp: plt.show()
    ...
    datasaver.save_figure(fig_trials, f"{mse_score}_{r2_tot}_{r2_proj}_LC_random_trials.png")
    if disp: plt.show()
